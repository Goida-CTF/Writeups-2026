...
def words_quaternion(bts):
    words = [(bts[i] << 8) + bts[i + 1] for i in range(0, len(bts), 2)]
    return sum(x * y for x, y in zip(words, (1, i, j, k)))

key = secrets.token_bytes(16)
blocks = [words_quaternion(key[i:i+8]) for i in range(0, len(key), 8)]
