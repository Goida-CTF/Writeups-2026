...
def quantize(x):
    return np.clip(np.floor(np.abs(x) * 64.0), 0, 255).astype(np.uint8)
...
def craft_network():
    ...
    DELTA = 1.0 / (8 * 64)  # 1/512

    delta = np.ones(HIDDEN_DIM, dtype=np.float32) * DELTA
    # = 1/512 ≈ 0.001953125
    ...
    output_q = quantize(output)
    input_q = quantize(secret_input)
    k = output_q ^ input_q
