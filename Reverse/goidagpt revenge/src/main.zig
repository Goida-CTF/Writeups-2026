...
inline fn quantize(x: f32) u8 {
    const scaled = @floor(@abs(x) * 64.0);
...
fn derive_key(output: [IO_DIM]f32, input: [IO_DIM]f32) [IO_DIM]u8 {
    ...
    for (0..IO_DIM) |i| {
        const qo = quantize(output[i]);
        const qi = quantize(input[i]);
        key[i] = qo ^ qi;
    }
    return key;
}
...
fn solve(key: [IO_DIM]u8, allocator: std.mem.Allocator) !?[]u8 {
    ...
    if (std.mem.startsWith(u8, flag, "goidactf{") and flag[flag.len - 1] == '}' and std.mem.indexOfScalar(u8, flag, 0) == null) { //TODO
        return flag;
    }
